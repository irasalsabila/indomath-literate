...
def load_mathstral_model():
    """Loads the Mathstral-7b-v0.1 model using AutoTokenizer and AutoModelForCausalLM."""
...
def run_inference(df_inference_results, model, tokenizer, device):
    """Generates answers using the Mathstral model."""
    ...
    for prompt in final_prompts:
        ...
        tokenized_prompt = tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_dict=True, return_tensors="pt").to(device)
...
def main():
    ...
    model, tokenizer = load_mathstral_model()
    ...
    for test_index in range(args.num_tests):
        ...
        model.to(device)
        df_inference_results = run_inference(df_inference_results, model, tokenizer, device)
        ...
        df_revised_consistency = perform_self_revision(
            df_consistency, df_inference_results, tokenizer, model, device,
        ...
        df_final.to_csv(f"results_mathstral/df_final_results_test_{test_index+1}.csv", index=False)
        print(f"✅ df_final saved to results_mathstral/df_final_results_test_{test_index+1}.csv")

        print("Computing metrics...")
        results = compute_metrics(df_final)

        # Store metrics for this test iteration
        results["test_index"] = test_index + 1
        ...
        print(f"\n✅ Finished Test {test_index + 1}/{args.num_tests}\n")

    metrics_file = "results_mathstral/metrics_results.csv"
    with open(metrics_file, "w", newline="") as csvfile:
        fieldnames = ["test_index", "total_responses", "correct_responses", "correct_after_revision",
                      "final_answer_accuracy", "self_revision_rate", "perplexity_per_language", 
                      "logical_errors", "numerical_errors", "revision_attempts"]
