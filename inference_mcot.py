...
MODEL_NAME = "laihuiyuan/mCoT"
LANGUAGES = ["en", "es", "ja", "th", "bn", "te"]
# Load models
def load_mcot_model():
    """Loads the mCoT model and tokenizer."""
    max_seq_length = 2048
    # dtype = None
    dtype = torch.float16  # Enable FP16 mixed precision
    load_in_4bit = True
...
# Prepare training and test questions
...
# Retrieve top-k few-shot examples
...
def run_inference(df_inference_results, model_mcot, tokenizer, device):
    """Generates answers using the mCoT model."""
    ...
    for prompt in final_prompts:
        ...
        outputs = model_mcot.generate(input_ids=inputs.input_ids, max_new_tokens=128, use_cache=True)
        decoded_text = tokenizer.batch_decode(outputs, skip_special_tokens=True)
        generated_responses.append(decoded_text[0])
...
def perform_self_revision(df_consistency, df_inference_results, tokenizer, model_mcot, device, 
                          negative_prompts, update_wait_messages, update_wait_again, max_waits=5, max_total_waits=15):
    """
    Performs self-revision for incorrect responses using iterative refinement, ensuring input lengths are within limits.
    """
    ...
    # Get incorrect rows that need self-revision
    ...
    for index, row in incorrect_rows.iterrows():
        ...
        prompt_template = negative_prompts.get(lang, negative_prompts["en"])  # Default to English if missing

        wait_count = 0
        is_correct = False

        while not is_correct and wait_count < max_waits:
            # Format the multilingual negative prompt
            full_prompt = prompt_template.format(question=question, original_response=original_response, update_wait1=update_wait)

            # Tokenize and check length
            tokenized_prompt = tokenizer(full_prompt, return_tensors="pt", truncation=False)
            token_length = tokenized_prompt["input_ids"].shape[1]

            # If it's too long, perform smart truncation
            if token_length > 2048:
                # Keep the question, shorten the original response, and keep only one example
                shortened_response = " ".join(original_response.split()[:300])  # Keep only the first 300 words
                truncated_prompt = prompt_template.format(question=question, original_response=shortened_response, update_wait1=update_wait)
            else:
                truncated_prompt = full_prompt  # Use the full prompt if it's already within the limit

            # Tokenize again with truncation enforced
            inputs = tokenizer(truncated_prompt, return_tensors="pt", padding=True, truncation=True, max_length=2048).to(device)

            # Generate response
            outputs = model_mcot.generate(input_ids=inputs.input_ids, max_new_tokens=128, use_cache=True)
            revised_response = tokenizer.batch_decode(outputs, skip_special_tokens=True)[0]
            ...
            # Check if self-revision produced the correct answer
            is_correct = correct_answer in numbers

            if not is_correct:
                wait_count += 1

                # If max retries reached, apply the final retry message
                if wait_count == max_waits:
                    update_wait += " " + update_wait_again.get(lang, "I made a mistake; try another way…")
                elif wait_count < max_total_waits:  # Keep adding "Wait..." until max_total_waits is reached
                    update_wait += " " + update_wait_messages.get(lang, "Wait …")
                else:
                    break  # Stop retrying after max_total_waits
            else:
                break

        # Determine the new status
        new_status = "Correct" if is_correct else f"Incorrect"
    ...
    # Create DataFrame for self-revision results
...
def merge_consistency_results(df_consistency, df_revised_consistency):
    ...
    # Ensure "expected_answer" exists in both dataframes before merging
...
def compute_perplexity(text):
    ...
    """
    tokens = text.split()  # Tokenize using whitespace (simplified approach)
    token_counts = Counter(tokens)  # Count occurrences of each token
    total_tokens = len(tokens)

    if total_tokens == 0:
        return float("inf")  # Avoid division by zero (empty text)
...
def main():
    ...
    model_mcot, tokenizer = load_mcot_model()
    ...
    # List to store metrics for each test run
    all_metrics = []
    ...
    for test_index in range(args.num_tests):
        ...
        model_mcot.to(device)
        df_inference_results = run_inference(df_inference_results, model_mcot, tokenizer, device)
        ...
        df_revised_consistency = perform_self_revision(
            df_consistency, df_inference_results, tokenizer, model_mcot, device,
        ...
        df_final.to_csv(f"results_mcot/df_final_results_test_{test_index+1}.csv", index=False)
        print(f"✅ df_final saved to results_mcot/df_final_results_test_{test_index+1}.csv")
    ...
    metrics_file = "results_mcot/metrics_results.csv"
