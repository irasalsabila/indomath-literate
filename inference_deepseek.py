...
def load_deepseek_model():
    """Loads the DeepSeek-Math-7B-Instruct model and tokenizer."""
...
def run_inference(df_inference_results, model, tokenizer, device):
    """Generates answers using the DeepSeek model."""
...
def main():
    ...
    for test_index in range(args.num_tests):
        ...
        df_final.to_csv(f"results_deepseek/df_final_results_test_{test_index+1}.csv", index=False)
        print(f"✅ df_final saved to results_deepseek/df_final_results_test_{test_index+1}.csv")
    ...
    metrics_file = "results_deepseek/metrics_results_deepseek.csv"
